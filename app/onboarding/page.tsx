        ...
        {docs.map((d, i) => (
          ...
          >
            <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-sm shrink-0 ${
              d.done ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-white/20'
            }`}>
              {d.done ? '✓' : i + 1}
            </div>
          ...
          <div className="flex items-center gap-2 mb-8">
            {STEPS.map((s, i) => (
              <div key={s.id} className="flex items-center gap-2 flex-1">
                <div
                  className={`h-7 w-7 rounded-lg flex items-center justify-center text-xs font-bold transition-all duration-300 shrink-0 ${
                    step > s.id
